import React from 'react';
import './Organizers.css';

const Organizers = () => {
  const organizers = [
    {
      name: 'Rahul Kothuri',
      role: 'AWS Cloud Clubs Captain',
      image: '👨‍💼'
    },
    {
      name: 'Sidda Deepika',
      role: 'Lead Organiser',
      image: '👩‍💼'
    },
    {
      name: 'Balu Amareswar',
      role: 'Technical Lead',
      image: '👨‍💻'
    },
    {
      name: 'R Charishma',
      role: 'Registration Management',
      image: '👩‍💼'
    },
    {
      name: 'Geepalem Leena',
      role: 'Registration Management',
      image: '👩‍💼'
    }
  ];

  return (
    <section className="organizers" id="organizers">
      <div className="organizers-container">
        <h2 className="section-title">
          <span className="highlight">Organizers</span>
        </h2>
        <p className="organizers-subtitle">
          Meet the dedicated AWS Student Community core team driving the mission forward
        </p>

        <div className="organizers-grid">
          {organizers.map((organizer, index) => (
            <div key={index} className="organizer-card">
              <div className="organizer-image">
                <div className="organizer-avatar">{organizer.image}</div>
              </div>
              <h3 className="organizer-name">{organizer.name}</h3>
              <p className="organizer-role">{organizer.role}</p>
              <div className="organizer-social">
                <a href="#" className="linkedin-btn">
                  <svg width="24" height="24" viewBox="0 0 24 24" fill="white">
                    <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z"/>
                  </svg>
                </a>
              </div>
            </div>
          ))}
        </div>
      </div>
    </section>
  );
};

export default Organizers;
